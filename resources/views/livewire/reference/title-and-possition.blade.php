    ...
    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-8">
        ...
        <nav class="flex-shrink-0" aria-label="Breadcrumb">
            <ol class="flex items-center space-x-2 text-sm text-zinc-500 dark:text-zinc-400">
                ...
                <li>
                    <span class="font-medium text-zinc-900 dark:text-zinc-100">Divisi and Jabatan</span>
                </li>
            </ol>
        </nav>
    </div>
    ...
    <div class="mb-6">
        <nav class="flex space-x-8" aria-label="Tabs">
            <button
                ...
                class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'positions' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300' }}"
            >
                Jabatan
            </button>
            <button
                wire:click="switchTab('divisions')"
                class="whitespace-nowrap py-2 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'divisions' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-zinc-500 hover:text-zinc-700 hover:border-zinc-300 dark:text-zinc-400 dark:hover:text-zinc-300' }}"
            >
                Divisi
            </button>
        </nav>
                                ...
                                <tr wire:key="position-{{ $position->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <flux:button wire:click="editPosition({{ $position->id }})" size="sm" variant="filled" icon="pencil">
                                                Edit
                                            </flux:button>
                                            <flux:button wire:click="deletePosition({{ $position->id }})" size="sm" variant="danger" icon="trash">
                                                Hapus
                                            </flux:button>
                                        </div>
                                    </td>
                        ...
                        <tbody class="bg-white dark:bg-zinc-900 divide-y divide-zinc-200 dark:divide-zinc-700">
                            @foreach($divisions as $index => $division)
                                <tr wire:key="division-{{ $division->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-800">
                                    ...
                                    <td class="px-4 py-2 whitespace-nowrap text-sm font-medium">
                                        <div class="flex items-center space-x-2">
                                            <flux:button wire:click="editDivision({{ $division->id }})" size="sm" variant="filled" icon="pencil">
                                                Edit
                                            </flux:button>
                                            <flux:button wire:click="deleteDivision({{ $division->id }})" size="sm" variant="danger" icon="trash">
                                                Hapus
                                            </flux:button>
                                        </div>
                                    </td>
